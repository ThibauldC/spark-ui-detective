"""Case 2 bad run: shuffle a small route dimension instead of broadcasting it."""
...
spark.sparkContext.setJobDescription("CASE 2 BAD: route enrichment join")
...
(summary.write.format("delta").mode("overwrite").option("overwriteSchema", "true")
 .save(OUTPUT_PATH))
print(f"CASE 2 BAD: {time.perf_counter() - started:.1f}s")
