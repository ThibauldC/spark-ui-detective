"""Case 2 fixed run: stream the same partitions with bounded Python state."""
...
def profile_partition(rows):
    # One dictionary at a time; retained state is one counter per column.
...
def main():
    ...
    spark.sparkContext.setJobDescription("CASE 2 FIXED: streaming Python partition profiling")
    ...
    print(f"CASE 2 FIXED: {time.perf_counter() - started:.1f}s, {PARTITIONS} profiling tasks")
