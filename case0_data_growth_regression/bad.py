"""Case 0 bad run: full taxi history with too few shuffle partitions."""
...
spark.sparkContext.setJobDescription("CASE 0 BAD: data growth and spill")
...
(summary.write.format("delta").mode("overwrite").option("overwriteSchema", "true")
 .save(OUTPUT_PATH))
print(f"CASE 0 BAD: {time.perf_counter() - started:.1f}s, {SHUFFLE_PARTITIONS} shuffle partitions")
