...
```bash
# Run from the Slidev project directory.
eval "$(fnm env)" && corepack pnpm create slidev  # Create project
eval "$(fnm env)" && corepack pnpm run dev        # Start dev server (opens http://localhost:3030)
eval "$(fnm env)" && corepack pnpm run build      # Build static SPA
eval "$(fnm env)" && corepack pnpm run export     # Export to PDF (requires playwright-chromium)
```

**Verify**: After `corepack pnpm run dev`, confirm slides load at `http://localhost:3030`. After `corepack pnpm run export`, check the output PDF exists in the project root.
